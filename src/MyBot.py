...
def enemy_get_dangered_icebergs(game, p_amount=35, danger_range=20):
    ...
    for our_iceberg in game.get_my_icebergs():
        for enemy_iceberg in game.get_enemy_icebergs():
            if our_iceberg.get_turns_till_arrival(
                    enemy_iceberg) <= danger_range and enemy_iceberg.penguin_amount <= p_amount:
    ...
    for enemy_group in enemy_penguin_groups:
        if enemy_group.destination in game.get_my_icebergs():
            if enemy_group.turns_till_arrival <= distance:
                if all_groups_to_dest(game,
                                      enemy_group.destination) >= enemy_group.destination.penguin_amount + enemy_group.turns_till_arrival * enemy_group.destination.level:
...
def can_attack(game, our_iceberg, enemy_iceberg, additional=0, is_not_neutral=1):
    ...
    if is_not_neutral == 1:
        for eny_ice in game.get_enemy_icebergs():
            if eny_ice.get_turns_till_arrival(enemy_iceberg) <= our_iceberg.get_turns_till_arrival(
                    enemy_iceberg) and enemy_iceberg != eny_ice:
    ...
    can_take = after_attacked_balance + additional > enemy_defense_sum + enemy_iceberg.penguin_amount + our_iceberg.get_turns_till_arrival(
        enemy_iceberg) * is_not_neutral * enemy_iceberg.level
...
def get_send_to_attack(game, our_iceberg, enemy_iceberg):
    ...
    for group in game.get_enemy_penguin_groups():
        if group.destination == enemy_iceberg:
            enemy_defense_sum += group.penguin_amount
    amount = enemy_iceberg.penguin_amount + our_iceberg.get_turns_till_arrival(
        enemy_iceberg) * enemy_iceberg.penguins_per_turn + 1 + enemy_defense_sum
    return amount
...
def upgrade_val(game, iceberg):
    ...
    final_val = (iceberg.penguin_amount * 1.0 / iceberg.upgrade_cost) * (
                iceberg.penguins_per_turn + iceberg.upgrade_value) * cost_eff * 1.0
    ...
    for group in game.get_enemy_penguin_groups():
        if group.destination in game.get_neutral_icebergs():
            ...
            if nearest_base.get_turns_till_arrival(group.destination) > group.turns_till_arrival:
                if icebergs_balance[nearest_base] >= our_abs(all_groups_to_dest(game,
                                                                                group.destination) - group.destination.penguin_amount) + nearest_base.get_turns_till_arrival(
                        group.destination):
                    if nearest_base.can_send_penguins(group.destination, our_abs(all_groups_to_dest(game,
                                                                                                    group.destination) - group.destination.penguin_amount) + nearest_base.get_turns_till_arrival(
                            group.destination)):
                        smart_send(nearest_base, group.destination, our_abs(all_groups_to_dest(game,
                                                                                               group.destination) - group.destination.penguin_amount) + nearest_base.get_turns_till_arrival(
                            group.destination))
...
def defend(game):
    ...
    """
    global icebergs_balance
    need_help = {iceberg: [] for iceberg in game.get_my_icebergs() if
                 icebergs_balance[iceberg] <= 0}  # { need_help_iceberg: [possible_helpers] }
    helps_icebergs = {iceberg: (0, []) for iceberg in game.get_my_icebergs() if
    ...
    for need_help_iceberg, possible_helps in need_help.iteritems():
        ...
        for iceberg in game.get_my_icebergs():
            ...
            if penguin_amount_in_n_turns(game, need_help_iceberg, iceberg.get_turns_till_arrival(iceberg)) > 0 and \
                    icebergs_balance[iceberg] > 0 \
    ...
    for iceberg, defenders in defenders_devision.iteritems():
        ...
        for defender in defenders:
            if icebergs_balance[defender] + icebergs_balance[iceberg] > 0 and penguin_amount_in_n_turns(game, iceberg,
                                                                                                        iceberg.get_turns_till_arrival(
                                                                                                                defender)) > 0:
...
def split_amount_for_send(icebergs, amount, game):
    ...
    amount_by_iceberg = {iceberg: (
    all_groups_to_dest_minus_distances(game, iceberg), all_groups_to_dest_minus_distances(game, iceberg) / sum_balance)
        ...
        while not to_be_sent >= amount:
            for iceberg in icebergs:
                if to_be_sent >= amount:
                    break
                if to_send_by_iceberg[iceberg] + 1 <= amount_by_iceberg[iceberg] and icebergs_balance[iceberg] - \
...
def old_ko(game, base):
    ...
    for fire in game.get_my_icebergs():
        if fire.can_send_penguins(ice1, icebergs_balance[
            fire]) and sum > ice1.penguin_amount + ice1.level * farest_iceberg.get_turns_till_arrival(ice1):
...
def get_iceberg_with_nearest_enemy(game):
    ...
    nearest_range = sorted(game.get_enemy_icebergs(), key=lambda x: x.get_turns_till_arrival(nearest_ice))[
        0].get_turns_till_arrival(nearest_ice)
    for ice in icebergs:
        curr_range = sorted(game.get_enemy_icebergs(), key=lambda x: x.get_turns_till_arrival(ice))[
            0].get_turns_till_arrival(ice)
...
def transfer_to_closest_to_target(game, target):
    ...
    if (1 == 2 and not all([ice.level == 4 for ice in game.get_my_icebergs()])) or penguin_produce(game,
                                                                                                   game.get_myself()) < penguin_produce(
            game, game.get_enemy()):
    ...
    helpers = [(ice, amount) for ice, amount in iceberg_sendable.iteritems() if
               ice.level <= 4 and ice != nearest_to_target]
    if len(helpers) < 2:
        return
    helpers = sorted(helpers, key=lambda x: risk_heuristic(game, x[0]))
...
def tie_breaker(game):
    if not game.turn > game.max_turns * 28 / 30 or len(game.get_neutral_icebergs()) == 0:
        return
    attackers = [iceberg for iceberg in game.get_my_icebergs() if
                 icebergs_balance[iceberg] > 0 and icebergs_state[iceberg]]
    if len(attackers) == 0:
    ...
    if sorted(attackers, key=lambda x: x.get_turns_till_arrival(target), reverse=True)[0].get_turns_till_arrival(
            target) == game.max_turns - game.turn + 1:
...
def do_turn(game):
    ...
    our_pengs_sum = sum([ice.penguin_amount for ice in game.get_my_icebergs()]) + sum(
        [group.penguin_amount for group in game.get_my_penguin_groups()])
    enemy_pengs_sum = sum([eny_ice.penguin_amount for eny_ice in game.get_enemy_icebergs()]) + sum(
        [group.penguin_amount for group in game.get_enemy_penguin_groups()])

    icebergs_state = {iceberg: True for iceberg in game.get_my_icebergs()}
    icebergs_balance = {iceberg: all_groups_to_dest_minus_distances(game, iceberg) for iceberg in
    ...
    if base.level == 1:
        ...
        if risk_more_than_zero:
            ...
            if to_upgrade.can_upgrade() and not to_upgrade.already_acted and all_groups_to_dest_minus_distances(game,
                                                                                                                to_upgrade) > to_upgrade.upgrade_cost:
                to_upgrade.upgrade()
        ...
        our_conquered_icebergs = game.get_my_icebergs() + [eny_ice for eny_ice in game.get_enemy_icebergs() if
                                                           enemy_balance[eny_ice] < 0]
        if len(our_conquered_icebergs) < 5 and len(game.get_neutral_icebergs()) != 0:
        ...
        if penguin_produce(game, game.get_myself()) >= 7:
            enemy_dangered = enemy_get_dangered_icebergs(game)
            if enemy_dangered != []:
                game.debug("YAYA ---> " + " our_penguin_amount = " + str(
                    enemy_dangered[0][0].penguin_amount) + " enemy_penguin_amount = " + str(
                    enemy_dangered[0][1].penguin_amount) +
                           " Turns till enemy = " + str(
                    enemy_dangered[0][0].get_turns_till_arrival(enemy_dangered[0][1]))
            ...
            if enemy_dangered != []:
                for icebergs in enemy_dangered:
                    if can_attack(game, icebergs[0], icebergs[1]):
                        game.debug("Attacking. . . | " + str(icebergs[0].penguin_amount) + " | " + str(
                            icebergs[1].penguin_amount))
